...
---
-- The Lua binding of Kyoto Cabinet.
--
module("kyotocabinet", package.seeall)


---
-- Module object returned by the "require" expression.
-- @class table
-- @name kc
...
function visitor:visit_full(key, value) end
...
-- @param step true to move the cursor to the next record, or false for no move.  If it is omitted, false is specified.
-- @return true on success, or false on failure.
function cursor:set_value(value, step) end
...
function db:replace(key, value) end
...
function db:append(key, value) end
...
function db:increment(key, num, orig) end
...
function db:increment_double(key, num, orig) end
...
-- @return true on success, or false on failure.
function db:cas(key, oval, nval) end
