...
/* function prototypes */
int luaopen_kyotocabinet(lua_State* lua);
...
static int kc_atoi(lua_State* lua);
static int kc_atoix(lua_State* lua);
static int kc_atof(lua_State* lua);
static int kc_hash_murmur(lua_State* lua);
static int kc_hash_fnv(lua_State* lua);
static int kc_levdist(lua_State* lua);
static int kc_time(lua_State* lua);
static int kc_sleep(lua_State* lua);
static int kc_pack(lua_State* lua);
static int kc_unpack(lua_State *lua);
static int kc_split(lua_State *lua);
static int kc_codec(lua_State *lua);
static int kc_bit(lua_State *lua);
static int kc_strstr(lua_State *lua);
static int kc_strfwm(lua_State *lua);
static int kc_strbwm(lua_State *lua);
static int kc_regex(lua_State *lua);
static int kc_arraydump(lua_State *lua);
static int kc_arrayload(lua_State *lua);
static int kc_mapdump(lua_State *lua);
static int kc_mapload(lua_State *lua);
...
private:
  const char* visit_full(const char* kbuf, size_t ksiz,
    ...
    const char* rv = NOP;
    if (lua_pcall(lua_, argc, 1, 0) == 0) {
      if (lua_islightuserdata(lua_, -1)) {
        const char* trv = (const char*)lua_touserdata(lua_, -1);
        if (trv == kc::PolyDB::Visitor::REMOVE) rv = kc::PolyDB::Visitor::REMOVE;
      } else {
        size_t rsiz;
        const char* rbuf = lua_tolstring(lua_, -1, &rsiz);
        if (rbuf) {
          rv = rbuf;
          *sp = rsiz;
        }
...
/**
 * Wrapper of the MapReduce framework.
 */
 ...
 */
static void define_module(lua_State* lua) {
  setfieldstr(lua, "VERSION", kc::VERSION);
  setfieldfunc(lua, "atoi", kc_atoi);
  setfieldfunc(lua, "atoix", kc_atoix);
  setfieldfunc(lua, "atof", kc_atof);
  setfieldfunc(lua, "hash_murmur", kc_hash_murmur);
  setfieldfunc(lua, "hash_fnv", kc_hash_fnv);
  setfieldfunc(lua, "levdist", kc_levdist);
  setfieldfunc(lua, "time", kc_time);
  setfieldfunc(lua, "sleep", kc_sleep);
  setfieldfunc(lua, "pack", kc_pack);
  setfieldfunc(lua, "unpack", kc_unpack);
  setfieldfunc(lua, "split", kc_split);
  setfieldfunc(lua, "codec", kc_codec);
  setfieldfunc(lua, "bit", kc_bit);
  setfieldfunc(lua, "strstr", kc_strstr);
  setfieldfunc(lua, "strfwm", kc_strfwm);
  setfieldfunc(lua, "strbwm", kc_strbwm);
  setfieldfunc(lua, "regex", kc_regex);
  setfieldfunc(lua, "arraydump", kc_arraydump);
  setfieldfunc(lua, "arrayload", kc_arrayload);
  setfieldfunc(lua, "mapdump", kc_mapdump);
  setfieldfunc(lua, "mapload", kc_mapload);
}
...
static void define_err(lua_State* lua) {
  lua_newtable(lua);
  setfielduint(lua, "SUCCESS", kc::PolyDB::Error::SUCCESS);
  setfielduint(lua, "NOIMPL", kc::PolyDB::Error::NOIMPL);
  setfielduint(lua, "INVALID", kc::PolyDB::Error::INVALID);
  setfielduint(lua, "NOREPOS", kc::PolyDB::Error::NOREPOS);
  setfielduint(lua, "NOPERM", kc::PolyDB::Error::NOPERM);
  setfielduint(lua, "BROKEN", kc::PolyDB::Error::BROKEN);
  setfielduint(lua, "DUPREC", kc::PolyDB::Error::DUPREC);
  setfielduint(lua, "NOREC", kc::PolyDB::Error::NOREC);
  setfielduint(lua, "LOGIC", kc::PolyDB::Error::LOGIC);
  setfielduint(lua, "SYSTEM", kc::PolyDB::Error::SYSTEM);
  setfielduint(lua, "MISC", kc::PolyDB::Error::MISC);
...
static void define_vis(lua_State* lua) {
  lua_newtable(lua);
  lua_pushlightuserdata(lua, (void*)kc::PolyDB::Visitor::NOP);
  lua_setfield(lua, -2, "NOP");
  lua_pushlightuserdata(lua, (void*)kc::PolyDB::Visitor::REMOVE);
...
static int cur_call(lua_State* lua) {
  ...
  const char* vbuf;
  size_t ksiz, vsiz;
  char* kbuf = cur->cur->get(&ksiz, &vbuf, &vsiz, true);
  if (kbuf) {
...
static int cur_set_value(lua_State* lua) {
  ...
  bool step = argc > 2 ? lua_toboolean(lua, 3) : false;
  bool rv = cur->cur->set_value(vbuf, vsiz, step);
  lua_pushboolean(lua, rv);
  return 1;
}
...
static int cur_get(lua_State* lua) {
  ...
  const char* vbuf;
  size_t ksiz, vsiz;
  char* kbuf = cur->cur->get(&ksiz, &vbuf, &vsiz, step);
  if (kbuf) {
    lua_pushlstring(lua, kbuf, ksiz);
    lua_pushlstring(lua, vbuf, vsiz);
    delete[] kbuf;
...
static int cur_seize(lua_State* lua) {
  ...
  const char* vbuf;
  size_t ksiz, vsiz;
  char* kbuf = cur->cur->seize(&ksiz, &vbuf, &vsiz);
  if (kbuf) {
    lua_pushlstring(lua, kbuf, ksiz);
    lua_pushlstring(lua, vbuf, vsiz);
    delete[] kbuf;
...
static void define_db(lua_State* lua) {
  lua_newtable(lua);
  setfielduint(lua, "OREADER", kc::PolyDB::OREADER);
  setfielduint(lua, "OWRITER", kc::PolyDB::OWRITER);
  setfielduint(lua, "OCREATE", kc::PolyDB::OCREATE);
  setfielduint(lua, "OTRUNCATE", kc::PolyDB::OTRUNCATE);
  setfielduint(lua, "OAUTOTRAN", kc::PolyDB::OAUTOTRAN);
  setfielduint(lua, "OAUTOSYNC", kc::PolyDB::OAUTOSYNC);
  setfielduint(lua, "ONOLOCK", kc::PolyDB::ONOLOCK);
  setfielduint(lua, "OTRYLOCK", kc::PolyDB::OTRYLOCK);
  setfielduint(lua, "ONOREPAIR", kc::PolyDB::ONOREPAIR);
...
static int db_new(lua_State* lua) {
  ...
  if (argc > 1 && lua_islightuserdata(lua, 2)) {
    db->db = (kc::PolyDB*)lua_touserdata(lua, 2);
    db->light = true;
  } else {
    db->db = new kc::PolyDB;
    db->light = false;
...
static int db_delete_ptr(lua_State* lua) {
  ...
  if (lua_islightuserdata(lua, 2)) {
    kc::PolyDB* db = (kc::PolyDB*)lua_touserdata(lua, 2);
    delete db;
  }
  lua_pushnil(lua);
  return 1;
...
static int db_error(lua_State* lua) {
  ...
  SoftDB* db = (SoftDB*)lua_touserdata(lua, -1);
  if (!db) throwinvarg(lua, __KCFUNC__);
  kc::PolyDB::Error err = db->db->error();
...
static int db_open(lua_State* lua) {
  ...
  uint32_t mode = kc::PolyDB::OWRITER | kc::PolyDB::OCREATE;
...
static int db_set(lua_State* lua) {
  ...
  bool rv = db->db->set(kbuf, ksiz, vbuf, vsiz);
  lua_pushboolean(lua, rv);
  return 1;
}
...
static int db_add(lua_State* lua) {
  ...
  const char* vbuf = lua_tolstring(lua, 3, &vsiz);
  if (!db || !kbuf || !vbuf) throwinvarg(lua, __KCFUNC__);
  bool rv = db->db->add(kbuf, ksiz, vbuf, vsiz);
  lua_pushboolean(lua, rv);
  return 1;
}
...
static int db_replace(lua_State* lua) {
  ...
  const char* vbuf = lua_tolstring(lua, 3, &vsiz);
  if (!db || !kbuf || !vbuf) throwinvarg(lua, __KCFUNC__);
  bool rv = db->db->replace(kbuf, ksiz, vbuf, vsiz);
  lua_pushboolean(lua, rv);
  return 1;
}
...
static int db_append(lua_State* lua) {
  ...
  const char* vbuf = lua_tolstring(lua, 3, &vsiz);
  if (!db || !kbuf || !vbuf) throwinvarg(lua, __KCFUNC__);
  bool rv = db->db->append(kbuf, ksiz, vbuf, vsiz);
  lua_pushboolean(lua, rv);
  return 1;
}
...
static int db_increment(lua_State* lua) {
  ...
  num = db->db->increment(kbuf, ksiz, num, orint);
  if (num == kc::INT64MIN) {
    lua_pushnil(lua);
  } else {
    lua_pushnumber(lua, num);
...
static int db_increment_double(lua_State* lua) {
  ...
  double orig = argc > 3 ? lua_tonumber(lua, 4) : 0;
  num = db->db->increment_double(kbuf, ksiz, num, orig);
  if (kc::chknan(num)) {
    lua_pushnil(lua);
  } else {
    lua_pushnumber(lua, num);
...
static int db_cas(lua_State* lua) {
  ...
  if (!db || !kbuf) throwinvarg(lua, __KCFUNC__);
  bool rv = db->db->cas(kbuf, ksiz, ovbuf, ovsiz, nvbuf, nvsiz);
  lua_pushboolean(lua, rv);
  return 1;
}
...
static int db_seize(lua_State* lua) {
  ...
  if (!db || !kbuf) throwinvarg(lua, __KCFUNC__);
  size_t vsiz;
  char* vbuf = db->db->seize(kbuf, ksiz, &vsiz);
  if (vbuf) {
    lua_pushlstring(lua, vbuf, vsiz);
    delete[] vbuf;
  } else {
...
static int db_set_bulk(lua_State* lua) {
  ...
  int64_t rv = db->db->set_bulk(recs, atomic);
...
static int db_merge(lua_State* lua) {
  ...
  bool rv = db->db->merge(srcary, srcnum, (kc::PolyDB::MergeMode)mode);
  delete[] srcary;
  lua_pushboolean(lua, rv);
  return 1;
}
